...
ALTER TABLE cpc.customers
  ADD COLUMN IF NOT EXISTS enrollment_submission_id UUID REFERENCES cpc.enrollment_submissions(id);
...
ALTER TABLE cpc.nominees
  ADD COLUMN IF NOT EXISTS submission_id UUID REFERENCES cpc.enrollment_submissions(id);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'check_policy_or_submission'
      AND conrelid = 'cpc.nominees'::regclass
  ) THEN
    ALTER TABLE cpc.nominees
      ADD CONSTRAINT check_policy_or_submission
      CHECK (
