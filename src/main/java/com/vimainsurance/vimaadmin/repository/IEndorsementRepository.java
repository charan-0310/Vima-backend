...
public interface IEndorsementRepository extends JpaRepository<Endorsement, UUID>, JpaSpecificationExecutor<Endorsement> {
     ...
     */
    @Query(value = """
        SELECT e.* FROM cpc.endorsements e
        WHERE e.organization_id = :organizationId
        AND e.status::text = CAST(:status AS VARCHAR)
        AND e.approved_at >= COALESCE(CAST(:startDate AS TIMESTAMP), '1970-01-01'::TIMESTAMP)
        AND e.approved_at <= COALESCE(CAST(:endDate AS TIMESTAMP), '9999-12-31 23:59:59'::TIMESTAMP)
        """, nativeQuery = true)
